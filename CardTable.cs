    ...
    public class CardTable
    {
        ...
        public void ShowHand(Player player)
        {
            if (player.cards.Count > 0)
            {
                ...
                //Using a List<string> to collect the full names of the cards,
                //and then utilizing the String.Join method to generate a comma-separated string.
                List<string> cardNames = new List<string>();
                foreach (Card card in player.cards)
                {
                    cardNames.Add(card.name);
                }
                string cardsText = String.Join(", ", cardNames); 
                Console.Write(cardsText);
